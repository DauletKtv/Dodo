<template>
  <div class="flex items-center justify-between my-5 px-4 lg:px-0">
    <div class="flex">
      <svg class="h-11 mr-8" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 247 44.59">
      ...
      <div class="font-medium hidden lg:block">
        <p>Доставка пиццы <span class="text-main relative"><span @click="choose = 1" class="hover:cursor-pointer">{{choosedCitys.city}}</span>
        <span class="absolute z-2 left-0 top-5" v-if="choose === 1" >
        
<div v-for="obj of citys" :key="obj.id">
  <input type="radio" :id="obj.title" :value="obj.title" v-model="selected" v-on:click="hide(), sendData($event.target.value)" hidden>
<label :for="obj.title"  >{{obj.title}}</label>
    ...
    <div>
      <button @click="showCart = 1" class="bg-main text-white py-2 px-4 rounded-full">Корзина | <span v-if="activeOrder != null">{{ activeOrder.products.length }}</span><span v-if="activeOrder == null">0</span></button>
    ...
    <div v-if="showCart === 1" class="fixed bg-black w-full h-full top-0 left-0 bg-opacity-50">
      <div class="absolute overflow-y-auto right-0 top-0 w-1/4 bg-slate-200 h-full pt-5">
        <div class="flex justify-between px-5">
        <p class="px-3 text-xl font-medium">{{ activeOrder.products.length }} товара на {{ activeOrder.totalOfProducts }} тг.</p>
        <div class="hover: cursor-pointer" @click="showCart = 0">X</div>
        </div>
...
export default {
  ...
  computed: { ...mapGetters(['compTest']) },
  async mounted() {
    let res = await axios.get('http://localhost:3001/users');
    this.users = res.data

    let newOrder = await axios.get('http://localhost:3001/orders');
    this.orders = newOrder.data

    this.orders.forEach(element => {
      if(element.user_login === this.currentUser && element.status === false) {
        this.activeOrder = element
      }
    });
    let k = await axios.get('http://localhost:3001/city')
    this.citys = k.data;
    let i = await axios.get('http://localhost:3001/choosedCity')
    this.choosedCitys = i.data;
  },
  methods: {
    async sendData(e){
      await axios.put('http://localhost:3001/choosedCity', {city: e })
    },
     hide(){
      setTimeout(()=>{this.choose = 0
      this.choosedCitys.city = this.selected;
    ...
    endOrder () {
      this.orders.forEach(async (element) => {
        if (element.status === false && element.user_login == this.currentUser) {
          element.status = true
          await axios.put('http://localhost:3001/orders/' + element.id, element)
          this.$router.go()
        }
      });
    },
